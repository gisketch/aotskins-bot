		...
		if isOwner:
			...
			if not ctx.message.mentions:
				...
				if args[1].isdigit():
					...
					value = ""
					if len(args) < 3:
						await sendErrorEmbed(ctx, "You have to enter an argument! `[add/subtract]`")
						return	
					if args[2] == "add":
						value = "add"
						...
						await sendErrorEmbed(ctx, "That's not a valid argument! `[add/subtract]`")
						return
					if args[2] and value:
						# DO THE THING
